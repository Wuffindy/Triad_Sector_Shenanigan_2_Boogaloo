...
public sealed partial class SupermatterSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        // Triad: GravPulseEvent does not exist in this fork's GravityWellSystem (Monolith-line
        // refactor made GravPulse a direct method call with no event dispatch). OnGravPulse only
        // jittered the pulse period and played a sound, so the crystal's gravity well still works
        // off its YAML gravPulsePeriod. See OnGravPulse below (also commented out).
    ...
    // requires EE's throwing system edits, which I am not porting.
    ...
    private void OnItemInteract(EntityUid uid, SupermatterComponent sm, ref InteractUsingEvent args)
    {
        ...
        if (args.Handled ||
            ...
            return;
        // Triad: dropped a duplicated immune/godmode check that repeated the two conditions above
    ...
    */
    // Coyote End
    // Triad: GravPulseEvent removed in this fork's GravityWellSystem; handler disabled.
    // Restore if a grav-pulse hook is reintroduced (would re-enable pulse-period jitter + pull sound).
