...
// SPDX-FileCopyrightText: 2025 Kenzieclutch <greenjoegames@gmail.com>
//
...
public sealed partial class SupermatterComponent : Component
{
    ...
    /// <summary>
    /// Set once the delamination event has fired so it can only fire once.
    /// Triad: without this the delam switch re-runs every atmos tick if the crystal survives
    /// the event (the tesla ball can't consume it), spawning a new tesla ball each tick.
    /// </summary>
    [DataField]
    public bool Delaminated;
...
{
    // Triad: cached so the per-tick modifier passes don't allocate a fresh array via Enum.GetValues
    public static readonly Gas[] AllGases = Enum.GetValues<Gas>();
    ...
    public static readonly Dictionary<Gas, SupermatterGasFact> GasData = new()
    {
        ...
        // Triad: dropped upstream Gas.Helium and Gas.Respiron entries; those are
        // Einstein Engines custom gases absent from this fork's Gas enum.
        // They can never appear in a mixture here, and CalculateGasMixModifier defaults
        // missing gases to a zero SupermatterGasFact, so this is behavior-neutral.
    };
    ...
    public static float CalculateGasMixModifier(GasMixture mix, Func<SupermatterGasFact, float> getModifier)
    {
        ...
        foreach (var gasId in AllGases) // Triad: was Enum.GetValues<Gas>(), which allocates per call
